...
__all__ = ('YieldFrom', 'stop_iteration_value', 'Generator')
...
class YieldFrom(object):
    ...
    def __init__(self, iterable):
        ...
        #     self._next: Prepares to use built-in function next in
        #         __next__ for the first iteration on the iterator.
...
class Generator(object):
    ...
    def throw(self, type_, value=None, traceback=None):
        ...
        if yield_from is not None and not self._closed:
            try:
                ...
                self._closed = True
                raise
        if isinstance(type_, _BaseException):
            if value is not None:
                raise TypeError(_REDUNDANT_THROW_VALUE)
            value = type_
            type_ = type(value)
        elif (not isinstance(type_, type)
        or    not issubclass(type_, _BaseException)):
    ...
    def close(self):
        ...
        self._closed = True
        if self._yielded and not already_closed:
            try:
